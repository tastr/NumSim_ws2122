    ...
    do
    {
       ...
       for (int j = 2; j < j_max-2 ; j++)
        {   Gaussseidelterm1j= v*(discretization_.p(2,j)/dx2+ (discretization_.p(1,j-1)  + discretization_.p(1,j+1))/dy2-discretization_.rhs(1,j))/div_x;
            Gaussseideltermj1= v*((discretization_.p(j-1,1)+discretization_.p(j+1,1))/dx2+ discretization_.p(j,2)/   dy2-discretization_.rhs(j,1))/div_y;      
            
            discretization_.setP(1,j, (1-omega) * discretization_.p(1,j)+ omega*  Gaussseidelterm1j);
            discretization_.setP(j,1, (1-omega) * discretization_.p(j,1)+ omega*  Gaussseideltermj1);
            ...
            Gaussseideltermijend= v*((discretization_.p(j-1,jend)+discretization_.p(j+1,jend))/dx2+ discretization_.p(j,jend-1)/dy2-discretization_.rhs(j,jend))/div_y;

            discretization_.setP(iend,j, (1-omega) * discretization_.p(iend,j)+ omega* Gaussseideltermiendj);
            discretization_.setP(j,jend, (1-omega) * discretization_.p(j,jend)+ omega* Gaussseideltermijend);
