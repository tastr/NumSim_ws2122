...
deltat  = min3(min2(delta_x,delta_y)/4,delta_x/velocity_X.absmax(),delta_y/velocity_Y.absmax())*settings_.tau;
...
std::cout<< "wenn debugging erledigt ist UND DT anpassen bzw u,v als pointer auf das zeug setzen" <<std::endl;
...
//float deltat                  = min3(min2(delta_x,delta_y)/4,delta_x/u.absmax(),delta_y/v.absmax())*settings_.tau;
...
for (int j = 1; j < settings_.nCells[1]-1; j++)
{
    for (int i = 1; i < settings_.nCells[0]-1; i++)
    {
        ...
        RHS(i,j) =  ( (F(i,j)-F(i-1,j)) / delta_x + (G(i,j)-G(i,j-1)) / delta_y ) / deltat;
